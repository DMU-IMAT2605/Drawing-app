...
Octagon::Octagon(sf::Vector2i _pos)
{
	this->vxr.resize(9);
...
Octagon::Octagon(sf::Vector2i _pos, sf::Vector2f _size)
{
	this->vxr.resize(9);
	this->vxr.setPrimitiveType(sf::LinesStrip);
	...
	for (size_t i = 0; i < vxr.getVertexCount(); i++)
	{
		this->vxr[i] = sf::Vector2f(_pos.x + cos((PI / 4) * i) * _size.x, _pos.y + sin((PI / 4) * i) * _size.y);
...
void Octagon::render(sf::RenderTarget& _t)
{
	_t.draw(this->vxr);
}
